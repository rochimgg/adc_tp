...
def dir_spice2tex(path='.', final_path="", ltspice_directory=r'/Users/rgallo/sym', full_example=0):
    for filename in os.listdir(path):
        if os.path.isfile(os.path.join(path, filename)) and filename.endswith('.asc'):
            ...
            spice2tex(path + os.sep + filename, final_path, ltspice_directory=ltspice_directory,
...
def spice2tex(file_name_ltspice='Draft.asc', final_path="", ltspice_directory=r'/Users/rgallo/sym', full_example=0):
    ...
    for idx in enumerate(words):
        ...
        if idx[1][0] == 'SYMBOL':
            components_count = component_addition(idx[0], idx[1], ltspice_directory, component_list, node_list, words,
                                                  components_count, components_add_memory)
...
def spice2pgf(file_name_ltspice='Draft.asc', final_path="", ltspice_directory=r'/Users/rgallo/sym'):
    global components_count
    global components_add_memory
    ...
    if final_path:
        if not final_path[-1] == os.path.sep:
            final_path = final_path + os.path.sep
        save_file = final_path + file_name_ltspice.split(os.sep)[-1].split('.')[0] + r'.pgf'
    else:
        save_file = file_name_ltspice[0:-len(file_name_ltspice.split('.')[-1])] + r'pgf'
    ...
    for ind, t in enumerate(node_list):
        if not node_list[ind][3] and (len(node_list[ind][1]) + len(node_list[ind][2])) > 2:
            node_list[ind][3] = 'X' + str(current_node_index)
            xy = print_xy(node_list[ind][0])
            node_coordinates = node_coordinates + r'\draw ' + xy + ' to[short,-*] '
            node_coordinates += xy + ' coordinate (' + str(node_list[ind][3]) + ');\n'
            current_node_index = current_node_index + 1
...
def main():
    dir_spice2pgf(r'/Users/rgallo/Repositorios/adc_tp/ltspice', r'/Users/rgallo/Repositorios/adc_tp/latex/',
                  r'/Users/rgallo/sym')
